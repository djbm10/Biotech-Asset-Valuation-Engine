...
UNIVERSE: list[dict] = [
    ...
    # =========================================================================
    # WAVE 3 — Active monitors added 2026-05-26
    # =========================================================================

    dict(
        ticker="ARVN", company_id="co-arvn", asset_id="a-arvn",
        indication="ER+ breast cancer — ARV-471 (PROTAC ER degrader, Phase 3 VERITAC-2)",
        ranking_score=0.68, opportunity_score=0.65, conviction="high",
        catalyst="VERITAC-2 Phase 3 PFS readout vs fulvestrant in ER+/HER2- MBC",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="ARV-471 (vepdegestrant) meets PFS primary in VERITAC-2 vs fulvestrant, validating PROTAC ER degradation over antibody",
        company_name="Arvinas", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=1_800, cash_millions=650,
        top_5_likely_acquirers=["AZN", "NVS", "PFE", "BMY", "LLY"],
        mna_relevance_score=0.72, data_confidence_score=0.74,
        executive_view="Pfizer co-development partner. PROTAC first-in-class ER degrader with Phase 3 readout imminent. If VERITAC-2 is positive, whole-company or platform acquisition is likely. AZ (ER franchise) and Lilly (abemaciclib) are natural strategic fits.",
    ),
    dict(
        ticker="RVMD", company_id="co-rvmd", asset_id="a-rvmd",
        indication="KRAS G12C/X NSCLC/CRC — RMC-6236 (RAS(ON) multi-selective inhibitor, Phase 2/3)",
        ranking_score=0.70, opportunity_score=0.68, conviction="high",
        catalyst="RMC-6236 Phase 1/2 expansion cohort data in NSCLC and CRC",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="RMC-6236 achieves ≥30% ORR in KRAS G12X NSCLC demonstrating pan-RAS(ON) activity beyond G12C-only inhibitors",
        company_name="Revolution Medicines", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=4_200, cash_millions=1_200,
        top_5_likely_acquirers=["MRK", "BMY", "AZN", "RHHBY", "NVS"],
        mna_relevance_score=0.75, data_confidence_score=0.72,
        executive_view="Next-generation RAS(ON) inhibitor that addresses all KRAS/NRAS/HRAS mutations. If clinical data confirms pan-RAS activity, Revolution Medicines becomes the most strategically valuable oncology target of 2025-2026. Merck and BMS are natural acquirers given checkpoint combination logic.",
    ),
    dict(
        ticker="IMVT", company_id="co-imvt", asset_id="a-imvt",
        indication="gMG / CIDP / thyroid eye disease — batoclimab + IMVT-1402 (FcRn inhibitors)",
        ranking_score=0.66, opportunity_score=0.62, conviction="high",
        catalyst="IMVT-1402 Phase 2/3 readout in MG and thyroid eye disease vs batoclimab",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="IMVT-1402 demonstrates superiority to batoclimab on IgG reduction with clean albumin safety profile",
        company_name="Immunovant", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=2_400, cash_millions=700,
        top_5_likely_acquirers=["RHHBY", "AZN", "JNJ", "SAN", "LLY"],
        mna_relevance_score=0.68, data_confidence_score=0.70,
        executive_view="Second-generation FcRn platform with IMVT-1402 designed to avoid albumin reduction that limited batoclimab. Growing FcRn market (efgartigimod, rozanolixizumab, nipocalimab) makes Immunovant a consolidation target. Roche or AZ most likely given MG/autoimmune pipeline fit.",
    ),
    dict(
        ticker="FULC", company_id="co-fulc", asset_id="a-fulc",
        indication="facioscapulohumeral muscular dystrophy — losmapimod (p38α/β MAPK inhibitor, Phase 3 ReDUX4)",
        ranking_score=0.62, opportunity_score=0.58, conviction="medium",
        catalyst="ReDUX4 Phase 3 readout in FSHD1 and FSHD2",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="Losmapimod meets DUX4-driven biomarker and functional endpoint in FSHD Phase 3 enabling first-ever FSHD approval",
        company_name="Fulcrum Therapeutics", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=450, cash_millions=180,
        top_5_likely_acquirers=["BIIB", "SRPT", "NVS", "BMY", "REGN"],
        mna_relevance_score=0.65, data_confidence_score=0.62,
        executive_view="FSHD has no approved therapies and 1M patients globally. Losmapimod p38 inhibition reduces DUX4 expression, the root cause. Biogen and Sarepta (DMD expertise) are natural acquirers. Small cap with clear acquisition optionality post-Phase 3.",
    ),
    dict(
        ticker="SRRK", company_id="co-srrk", asset_id="a-srrk",
        indication="IgA nephropathy — BION-1301 (APRIL/BLyS dual inhibitor, Phase 3)",
        ranking_score=0.60, opportunity_score=0.56, conviction="medium",
        catalyst="Phase 3 APRIL-IgAN trial interim proteinuria data",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="BION-1301 achieves ≥35% proteinuria reduction at 9 months in IgA nephropathy supporting regulatory submission",
        company_name="Scholar Rock", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=600, cash_millions=220,
        top_5_likely_acquirers=["AMGN", "JNJ", "AZN", "NVS", "REGN"],
        mna_relevance_score=0.62, data_confidence_score=0.60,
        executive_view="IgA nephropathy is now a validated space (sparsentan approved, iptacopan, atrasentan) making Scholar Rock an acquisition candidate if BION-1301 Phase 3 shows biomarker signal. APRIL/BLyS dual targeting is mechanistically differentiated.",
    ),
    dict(
        ticker="EDIT", company_id="co-edit", asset_id="a-edit",
        indication="sickle cell disease — EBT-101 (CRISPR BCL11A enhancer, Phase 1/2)",
        ranking_score=0.56, opportunity_score=0.54, conviction="medium",
        catalyst="EBT-101 Phase 1/2 long-term follow-up data in SCD",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="EBT-101 achieves transfusion independence with durable HbF induction in SCD patients at 12+ months",
        company_name="Editas Medicine", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=350, cash_millions=220,
        top_5_likely_acquirers=["CRSP", "NTLA", "NVS", "BIIB", "REGN"],
        mna_relevance_score=0.58, data_confidence_score=0.55,
        executive_view="Editas has the most clinical CRISPR data behind CRISPR Tx/Vertex but market cap reflects doubts about IP position vs Cas9 Berkeley/Broad dispute. If EBT-101 shows durable efficacy, consolidation with CRISPR Tx (which won Cas9 patent) or Novartis gene therapy platform is plausible.",
    ),
    dict(
        ticker="BEAM", company_id="co-beam", asset_id="a-beam",
        indication="sickle cell disease — BEAM-101 (base editing BCL11A, Phase 1/2 BEACON)",
        ranking_score=0.65, opportunity_score=0.62, conviction="high",
        catalyst="BEACON Phase 1/2 readout: HbF induction and vaso-occlusive crisis reduction",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="BEAM-101 achieves ≥70% HbF induction with transfusion independence in BEACON SCD cohort",
        company_name="Beam Therapeutics", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=1_200, cash_millions=600,
        top_5_likely_acquirers=["NVS", "REGN", "BIIB", "VRTX", "LLY"],
        mna_relevance_score=0.70, data_confidence_score=0.65,
        executive_view="Base editing avoids double-strand DNA breaks (safer than CRISPR). BEACON SCD and BEAM-302 (alpha-1 AT) are near-term catalysts. Novartis (gene therapy leader) and Vertex (casgevy franchise) are the most logical acquirers — base editing could be the next generation of what CRISPR Tx built.",
    ),
    dict(
        ticker="KYMR", company_id="co-kymr", asset_id="a-kymr",
        indication="atopic dermatitis / hidradenitis suppurativa — KT-474 (IRAK4 PROTAC degrader, Phase 2)",
        ranking_score=0.64, opportunity_score=0.61, conviction="high",
        catalyst="KT-474 Phase 2 EMERGE readout in atopic dermatitis and HS",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="KT-474 achieves ≥60% EASI improvement in atopic dermatitis at week 16 with tolerability advantage over JAK inhibitors",
        company_name="Kymera Therapeutics", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=1_400, cash_millions=550,
        top_5_likely_acquirers=["AZN", "AbbVie", "LLY", "REGN", "SAN"],
        mna_relevance_score=0.68, data_confidence_score=0.68,
        executive_view="Sanofi collaboration partner. IRAK4 degradation blocks innate immune signaling — oral, once-daily, no steroidal AEs. If KT-474 shows clear superiority over JAKi in AD/HS, AbbVie (Rinvoq franchise) or Regeneron/Sanofi (Dupixent franchise) would be natural acquirers to add an oral mechanism.",
    ),
    dict(
        ticker="KROS", company_id="co-kros", asset_id="a-kros",
        indication="Friedreich ataxia — omaveloxolone (Skyclarys, approved; Nrf2 activator)",
        ranking_score=0.60, opportunity_score=0.56, conviction="medium",
        catalyst="Omaveloxolone real-world effectiveness data and international approvals",
        claim_type=ClaimType.REGULATORY_PATHWAY,
        claim_assertion="Omaveloxolone (Skyclarys) achieves EU and ex-US regulatory approvals expanding addressable FA patient population",
        company_name="Keros Therapeutics", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=750, cash_millions=300,
        top_5_likely_acquirers=["BIIB", "PTC", "NVS", "RHHBY", "SAN"],
        mna_relevance_score=0.62, data_confidence_score=0.65,
        executive_view="Skyclarys (omaveloxolone) is the first approved Friedreich ataxia therapy — rare neurological disease with no prior treatments. Ex-US approvals expand the commercial opportunity. Biogen (neurology rare disease) and Roche (neurology portfolio) are natural acquirers for a de-risked rare CNS asset.",
    ),
    dict(
        ticker="VKTX", company_id="co-vktx", asset_id="a-vktx",
        indication="MASH / obesity — VK2809 (THR-beta agonist, Phase 3 VOYAGE-II)",
        ranking_score=0.67, opportunity_score=0.64, conviction="high",
        catalyst="VK2809 Phase 3 VOYAGE-II histological endpoints in MASH",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="VK2809 achieves NASH resolution + fibrosis improvement in Phase 3 VOYAGE-II establishing Viking as a leading MASH acquiree",
        company_name="Viking Therapeutics", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=3_200, cash_millions=950,
        top_5_likely_acquirers=["NVO", "LLY", "AZN", "NVS", "RHHBY"],
        mna_relevance_score=0.72, data_confidence_score=0.70,
        executive_view="THR-beta agonism is the validated mechanism behind resmetirom (Rezdiffra). If VK2809 Phase 3 confirms histological endpoints, Viking becomes the most obvious MASH acquisition target. Novo (GLP-1 MASH program) and Lilly (tirzepatide MASH) would both benefit from adding a THR-beta agonist.",
    ),
    dict(
        ticker="AGIO", company_id="co-agio", asset_id="a-agio",
        indication="PKD / hereditary hemolytic anemia — mitapivat (Ph3 ACTIVATE-Kids) + AG-946 (SCD)",
        ranking_score=0.63, opportunity_score=0.60, conviction="medium",
        catalyst="ACTIVATE-Kids Ph3 readout in pediatric PKD; AG-946 Phase 2 SCD data",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="Mitapivat demonstrates hemoglobin improvement in pediatric PKD meeting primary endpoint for label expansion",
        company_name="Agios Pharmaceuticals", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=1_600, cash_millions=700,
        top_5_likely_acquirers=["AMGN", "NVS", "BIIB", "PFE", "REGN"],
        mna_relevance_score=0.65, data_confidence_score=0.68,
        executive_view="PKA kinase activator mechanism (mitapivat) validated in adult thalassemia and PKD. Pediatric label expansion and SCD (AG-946) would expand the franchise significantly. Agios sold its oncology assets to Servier in 2021; remaining rare blood disease platform is a clean acquisition target.",
    ),
    dict(
        ticker="IDYA", company_id="co-idya", asset_id="a-idya",
        indication="BRCA/HRR-mutant solid tumors — IDE161 (PARP1-selective) + darovasertib (PKC)",
        ranking_score=0.56, opportunity_score=0.52, conviction="medium",
        catalyst="IDE161 Phase 1/2 ORR data in BRCA-mutant solid tumors",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="IDE161 (PARP1-selective) achieves ≥40% ORR in BRCA-mutant solid tumors with reduced thrombocytopenia vs pan-PARP",
        company_name="Ideaya Biosciences", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=1_100, cash_millions=480,
        top_5_likely_acquirers=["AZN", "MRK", "GSK", "PFE", "NVS"],
        mna_relevance_score=0.60, data_confidence_score=0.60,
        executive_view="GSK co-development partner (darovasertib/IDE196). PARP1-selective IDE161 could offer PARP inhibitor efficacy with less myelosuppression. AstraZeneca (Lynparza) and Merck (Keytruda combination) are natural acquirers for the HRR portfolio.",
    ),
    dict(
        ticker="RCUS", company_id="co-rcus", asset_id="a-rcus",
        indication="solid tumors — domvanalimab (TIGIT) + zimberelimab (PD-1) + quemliclustat (CD73)",
        ranking_score=0.58, opportunity_score=0.55, conviction="medium",
        catalyst="STAR-221 Phase 3 interim (domvanalimab + zimberelimab vs pembrolizumab in PD-L1+ NSCLC)",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="Domvanalimab (TIGIT) + zimberelimab demonstrates ≥20% PFS improvement vs pembrolizumab in 1L PD-L1+ NSCLC",
        company_name="Arcus Biosciences", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=1_200, cash_millions=500,
        top_5_likely_acquirers=["GILD", "AZN", "BMY", "MRK", "REGN"],
        mna_relevance_score=0.65, data_confidence_score=0.62,
        executive_view="Gilead strategic partner with broad option to acquire. Domvanalimab is a key TIGIT bet in the post-tiragolumab era. If STAR-221 shows PFS improvement, Gilead's option activation becomes highly likely. Adenosine inhibition (AB928) is also differentiated for cold tumors.",
    ),
    dict(
        ticker="PMV", company_id="co-pmv", asset_id="a-pmv",
        indication="TP53 Y220C-mutant solid tumors — PC14586 (p53 reactivator, Phase 1/2 PYNNACLE)",
        ranking_score=0.60, opportunity_score=0.58, conviction="medium",
        catalyst="PYNNACLE Phase 1/2 TP53 Y220C expansion cohort ORR data",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="PC14586 achieves ≥25% ORR in TP53 Y220C-mutant solid tumors confirming mutant p53 reactivation as a viable oncology mechanism",
        company_name="PMV Pharmaceuticals", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=300, cash_millions=180,
        top_5_likely_acquirers=["AZN", "BMY", "MRK", "AMGN", "NVS"],
        mna_relevance_score=0.63, data_confidence_score=0.58,
        executive_view="TP53 mutation is the most common oncogenic driver in cancer; reactivation of Y220C mutant is a completely novel mechanism. Early Phase 1 ORR of 23% in pretreated patients is promising. Small cap with high optionality if broader TP53 landscape validates.",
    ),
    dict(
        ticker="DNLI", company_id="co-dnli", asset_id="a-dnli",
        indication="Parkinson's disease — DNL151/BIIB122 (LRRK2 inhibitor, Phase 2/3 LIGHTHOUSE)",
        ranking_score=0.64, opportunity_score=0.60, conviction="high",
        catalyst="LIGHTHOUSE Phase 2/3 biomarker and clinical readout in LRRK2 Parkinson's",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="DNL151 demonstrates significant pSer935-LRRK2 biomarker reduction and clinical signal in LIGHTHOUSE Phase 2/3",
        company_name="Denali Therapeutics", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=2_200, cash_millions=900,
        top_5_likely_acquirers=["BIIB", "RHHBY", "AZN", "LLY", "NVS"],
        mna_relevance_score=0.68, data_confidence_score=0.66,
        executive_view="Biogen co-development partner. LRRK2 kinase is the most druggable Parkinson's target. Denali's blood-brain-barrier Transport Vehicle (TV) platform differentiates it. If LIGHTHOUSE shows disease modification, whole-company acquisition by Biogen or Roche is most likely.",
    ),
    dict(
        ticker="ADMA", company_id="co-adma", asset_id="a-adma",
        indication="primary immunodeficiency / COVID-19 — ASCENIV (high-titer RSV/COVID IVIG)",
        ranking_score=0.54, opportunity_score=0.50, conviction="medium",
        catalyst="ASCENIV commercial ramp and potential FDA label expansion",
        claim_type=ClaimType.MARKET_REACTION_POSITIVE,
        claim_assertion="ASCENIV achieves >$200M annual revenue run rate driven by RSV/COVID high-titer IVIG differentiation in immunocompromised patients",
        company_name="ADMA Biologics", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=1_800, cash_millions=120,
        top_5_likely_acquirers=["CSL", "Takeda", "GRIFOLS", "BIIB", "GSK"],
        mna_relevance_score=0.62, data_confidence_score=0.65,
        executive_view="Commercial-stage plasma-derived IVIG company with differentiated high-titer products. CSL Behring or Takeda (plasma franchise) are logical acquirers to consolidate IVIG market share.",
    ),
    dict(
        ticker="KPTI", company_id="co-kpti", asset_id="a-kpti",
        indication="multiple myeloma / DLBCL — selinexor (Xpovio, XPO1 inhibitor, approved)",
        ranking_score=0.56, opportunity_score=0.52, conviction="medium",
        catalyst="Selinexor combination regimens Phase 3 data in newly diagnosed myeloma",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="Selinexor-based combination demonstrates PFS benefit in 1L myeloma expanding Xpovio label beyond relapsed/refractory",
        company_name="Karyopharm Therapeutics", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=250, cash_millions=100,
        top_5_likely_acquirers=["JNJ", "AMGN", "BMY", "AbbVie", "PFE"],
        mna_relevance_score=0.58, data_confidence_score=0.58,
        executive_view="Selinexor (Xpovio) is approved in R/R MM and DLBCL. If frontline myeloma combination data is positive, J&J (daratumumab franchise) or Amgen (carfilzomib) are natural acquirers. Low market cap relative to approved commercial stage makes it a potential value acquisition.",
    ),
    dict(
        ticker="PRTA", company_id="co-prta", asset_id="a-prta",
        indication="Alzheimer's disease — PRX012 (amyloid-clearance antibody, Phase 2)",
        ranking_score=0.58, opportunity_score=0.54, conviction="medium",
        catalyst="PRX012 Phase 2 amyloid PET clearance and safety data vs lecanemab benchmark",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="PRX012 achieves >80% amyloid plaque clearance at lower dose than lecanemab with ≤15% ARIA-E rate",
        company_name="Prothena", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=900, cash_millions=500,
        top_5_likely_acquirers=["BIIB", "LLY", "RHHBY", "AZN", "NVS"],
        mna_relevance_score=0.63, data_confidence_score=0.60,
        executive_view="Prothena has multiple CNS/rare disease programs. PRX012 is designed with SC delivery and improved safety profile vs IV lecanemab. If Phase 2 validates, Biogen or Lilly would likely acquire to add a subcutaneous amyloid antibody to their portfolios.",
    ),
    dict(
        ticker="RXRX", company_id="co-rxrx", asset_id="a-rxrx",
        indication="rare kidney / oncology — AI-discovered programs (REC-2282 NF2, CDG-RL101)",
        ranking_score=0.50, opportunity_score=0.48, conviction="medium",
        catalyst="Phase 1 IND clearance and FIH data for AI-derived programs",
        claim_type=ClaimType.POS_ABOVE_THRESHOLD,
        claim_assertion="Recursion's AI-discovered programs achieve IND clearance and early Phase 1 safety data validating ML-driven target identification",
        company_name="Recursion Pharmaceuticals", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=1_800, cash_millions=500,
        top_5_likely_acquirers=["NVS", "RHHBY", "AZN", "AMGN", "PFE"],
        mna_relevance_score=0.60, data_confidence_score=0.52,
        executive_view="Recursion is primarily a platform/AI company. Nvidia investment validates the compute approach. Novartis (Institute for BioMedical Research AI bet) or AstraZeneca (AI collaboration) would be the most logical platform acquirers.",
    ),
    dict(
        ticker="ZYME", company_id="co-zyme", asset_id="a-zyme",
        indication="HER2+ breast/gastric — zanidatamab (bispecific HER2 antibody) + ZW49 (bispecific ADC)",
        ranking_score=0.60, opportunity_score=0.57, conviction="medium",
        catalyst="Zanidatamab Phase 3 HERIZON-GEA-01 readout in HER2+ GEA",
        claim_type=ClaimType.ENDPOINT_MET,
        claim_assertion="Zanidatamab achieves PFS primary in HER2+ gastroesophageal adenocarcinoma establishing bispecific HER2 as new 1L standard",
        company_name="Zymeworks", exchange="NASDAQ", country="US",
        region="North America", company_type="target", target_type="target",
        market_cap_millions=700, cash_millions=350,
        top_5_likely_acquirers=["AZN", "RHHBY", "MRK", "BMY", "JNJ"],
        mna_relevance_score=0.65, data_confidence_score=0.62,
        executive_view="BeiGene collaboration. Zanidatamab dual HER2 epitope binding offers potentially superior efficacy to trastuzumab in GEA and biliary. AstraZeneca (T-DXd franchise) would benefit from adding a bispecific HER2 antibody; Roche (Herceptin/Perjeta) would not want a competitor acquiring it.",
    ),
]
